...
import './../css/UserConvertwayDashboard.css';
import {
  ResponsiveContainer,
  BarChart,
  Bar,
...
function UserConvertwayDashboard() {
  ...
  const fetchTopMerchants = async () => {
    try {
      const res = await fetch(
        `${BACKEND_BASE_URL}/user/convertway-high-loyalty-churn`,
        {
          method: 'GET',
          headers: {
  ...
  const fetchAvgLoyalHighChurnMerchants = async () => {
    try {
      const res = await fetch(
        `${BACKEND_BASE_URL}/user/convertway-average-loyalty-high-churn`,
        {
          method: 'GET',
          headers: {
            'ngrok-skip-browser-warning': 'true',
  ...
  return (
    <div className="convertway-dashboard container">
      <h2 className="dashboard-title">Convertway Summary</h2>
        ...
        {topMerchants.length > 0 ? (
          <ResponsiveContainer width="100%" height={500}>
            ...
            >
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="loyalty_score_convertway" fill="#8884d8" name="Loyalty Score" />
              <Bar dataKey="churn_rate_convertway" fill="#82ca9d" name="Churn Rate" />
            </BarChart>
          </ResponsiveContainer>
        ) : (
        ...
        {avgLoyalHighChurnMerchants.length > 0 ? (
          <ResponsiveContainer width="100%" height={500}>
            ...
            >
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="loyalty_score_convertway" fill="#ffc658" name="Loyalty Score" />
              <Bar dataKey="churn_rate_convertway" fill="#ff7300" name="Churn Rate" />
            </BarChart>
          </ResponsiveContainer>
        ) : (
...
export default UserConvertwayDashboard;
