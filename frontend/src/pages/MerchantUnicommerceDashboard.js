  ...
  const fetchUnicommerceScores = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/merchant/unicommerce-loyalty`,
        {
      ...
      if (response.data.success) {
        setLoyaltyScore(response.data.loyalty_score_unicommerce);
      } else {
  ...
  const fetchUnicommerceHistory = async () => {
    ...
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/merchant/unicommerce-loyalty-history`,
        { params: { email: auth.email } }
      );
